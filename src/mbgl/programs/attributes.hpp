...
template <class... Ps>
class PaintAttributes<style::PaintProperties<Ps...>> {
public:
    using Attributes = gl::Attributes<typename Ps::Attribute...>;

    using State = typename Attributes::State;
    using Values = typename Attributes::Values;

    template <class AllProperties>
    static Values allConstantValues(const AllProperties& properties) {
        return {
    ...
    template <class AllProperties>
    static std::function<void (std::size_t)> binder(const State& state, const AllProperties& properties) {
        return Attributes::binder(state, allConstantValues(properties));
    }
};
